...
val exclusiveStoreIds: Collection<UUID>? = ...Some code...
val orderByName: Boolean = ...Some code...
val orderByBookName: Boolean = ...Some code...
val orderByStoreName: Boolean = ...Some code...
...
// #1: Orthodromic join by the source code generated by KSP
where(table.books.store.name eq "MANNING")
 
// #2: Orthodromic join by low level API
where {
    table
        .joinList(Author::books)
        .joinReference(Book::store)
        .get(BookStore::name) eq "MANNING"
}

// #3: Reverse join by low level API
where {
    table
        .`←joinList`(Book::authors) // Reverse join: Author -> Book
        .`←joinReference`(BookStore::books) // Reverse join: Book -> BookStore
        .get(BookStore::name) eq "MANNING"
}
